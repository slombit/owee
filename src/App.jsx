...
const makeGroup = (title = "Nuevo grupo") => ({
  id: genId(), title, people: [], expenses: [],
  createdAt: today(), closed: false
});
...
export default function App() {
  ...
  const listeners = useRef({});   // groupId → unsubscribe fn
  const active    = groups.find(g => g.id === activeId);

  // ── Cargar grupos locales al iniciar ────────────────────────────────────────
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setGroups(parsed);
        // Re-suscribir a todos los grupos que ya teníamos
        parsed.forEach(g => subscribeToGroup(g.id));
      } catch {}
    }
  }, []);

  // ── Persistir lista de grupos localmente ────────────────────────────────────
  useEffect(() => {
    if (groups.length > 0) {
  ...
  // ── Suscribir a un grupo en Firebase (tiempo real) ──────────────────────────
  const subscribeToGroup = useCallback((groupId) => {
    if (listeners.current[groupId]) return; // ya suscripto
  ...
  const joinGroup = () => {
    ...
    onValue(groupRef, (snap) => {
      const data = snap.val();
      if (!data) {
        setJoinError("No encontré ese grupo. Revisá el código.");
        return;
      }
      setGroups(prev => {
  ...
  const deleteGroup = (id) => {
    ...
    // Solo borra de Firebase si sos quien lo creó (no tiene auth, así que lo borramos igual)
  ...
  const removeExpense = (eid) => updateActive(g => ({
    ...g, expenses: (g.expenses || []).filter(e => e.id !== eid)
  }));

  const toggleSplit = (pid) => setNewExp(e => ({
    ...e,
    splitWith: e.splitWith.includes(pid)
      ? e.splitWith.filter(i => i !== pid)
      : [...e.splitWith, pid]
  }));
  ...
  const shareGroup = useCallback(() => {
    ...
    msg += `📅 ${active.createdAt}\n`;
    msg += `👥 ${people.map(p => p.name).join(", ")}\n`;
    msg += `💰 Total: *$${total.toFixed(2)}*\n`;
    msg += `🔑 Código del grupo: \`${active.id}\`\n\n`;
  ...
  const Avatar = ({ name, color, bg, size = 36 }) => (
    <div style={{
      ...
      fontWeight: 700, fontSize: size * 0.38, color: color || "#333",
  ...
  return (
    <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: "#f7f7f5", position: "relative" }}>
      <style>{`
        ...
        .pill { border:1.5px solid #e8e8e5; background:white; color:#1a1a1a; border-radius:50px; padding:8px 16px; font-size:13px; font-weight:600; cursor:pointer; transition:all 0.15s; font-family:inherit; }
        .pill:hover { border-color:#1a1a1a; }
        .pill.active { background:#1a1a1a; color:white; border-color:#1a1a1a; }
        .btn { background:#1a1a1a; color:white; border:none; border-radius:14px; padding:14px; font-size:15px; font-weight:700; cursor:pointer; width:100%; font-family:inherit; transition:opacity 0.15s; }
        ...
        <div className="su" style={{ paddingBottom: 60 }}>
          ...
          <div style={{ padding: "60px 24px 28px", background: "#1a1a1a", borderRadius: "0 0 28px 28px" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button className="pill" style={{ background:"white", color:"#1a1a1a", fontSize:14, padding:"11px 20px" }} onClick={() => setShowNewModal(true)}>
                + Nuevo grupo
              </button>
              <button className="pill" style={{ background:"rgba(255,255,255,0.1)", border:"1.5px solid rgba(255,255,255,0.2)", color:"white", fontSize:14, padding:"11px 20px" }} onClick={() => setShowJoinModal(true)}>
                Unirse
              </button>
            </div>
          </div>
            ...
            <div style={{ padding:"56px 24px 0" }}>
              <div className="row" style={{ justifyContent:"space-between", marginBottom:6 }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <button
                    className="pill"
                    style={{ fontSize:12, padding:"6px 14px", background:copied?"#1a1a1a":"white", color:copied?"white":"#1a1a1a", borderColor:copied?"#1a1a1a":"#e8e8e5" }}
                    onClick={shareGroup}
                  >
                  ...
                  <button
                    className="pill"
                    style={{ fontSize:12, padding:"6px 14px", color:"#E8734A", borderColor:"#fdeee8" }}
              ...
              {editTitle ? (
                <input value={titleInput}
                  ...
                  onBlur={() => { updateActive(g => ({ ...g, title: titleInput || g.title })); setEditTitle(false); }}
                  onKeyDown={e => { if (e.key==="Enter") { updateActive(g => ({ ...g, title: titleInput||g.title })); setEditTitle(false); }}}
                  autoFocus
                ...
                <div onClick={() => { setTitleInput(active.title); setEditTitle(true); }}
                  style={{ fontSize:24,fontWeight:800,letterSpacing:"-0.5px",marginBottom:2,cursor:"text" }}>
                  {active.title}
                </div>
              )}

              {/* Código del grupo */}
              <div style={{ fontSize:11, color:"#bbb", marginBottom:2, fontFamily:"monospace", letterSpacing:0.5 }}>
                🔑 código: {active.id}
              </div>
              <div style={{ fontSize:13, color:"#bbb", marginBottom:20 }}>
                {(active.people||[]).length} personas · ${totalGastos(active).toFixed(2)} total
              </div>
            </div>
            ...
            <div className="row" style={{ padding:"0 24px" }}>
              {["gastos","personas","cuentas"].map(t => (
                <button key={t} onClick={() => setTab(t)} style={{
                  flex:1, padding:"12px 0", background:"none", border:"none", fontSize:14,
                  fontWeight:600, cursor:"pointer", fontFamily:"inherit",
                  color: tab===t ? "#1a1a1a" : "#bbb",
                  borderBottom: tab===t ? "2.5px solid #1a1a1a" : "2.5px solid transparent",
                          ...
                          {(active.people||[]).map(p => (
                            <button key={p.id} onClick={() => toggleSplit(p.id)}
                              ...
                              style={{ padding:"6px 14px",fontSize:13,
                                background:newExp.splitWith.includes(p.id)?"#1a1a1a":"white",
                                color:newExp.splitWith.includes(p.id)?"white":"#1a1a1a",
                                borderColor:newExp.splitWith.includes(p.id)?"#1a1a1a":"#e8e8e5"
                              }}
                            >{p.name}</button>
                          ))}
                          <button className="pill" style={{ padding:"6px 14px",fontSize:12 }}
                            onClick={() => setNewExp(x=>({...x,splitWith:(active.people||[]).map(p=>p.id)}))}>
                            Todos
                          </button>
                        </div>
                      </div>
        ...
        <div className="modal-bg fi" onClick={e=>{if(e.target===e.currentTarget)setShowNewModal(false)}}>
          <div className="modal">
            ...
            <button onClick={()=>setShowNewModal(false)}
              style={{ background:"none",border:"none",width:"100%",padding:"14px",fontFamily:"inherit",fontSize:14,color:"#bbb",cursor:"pointer",marginTop:4 }}>
              Cancelar
            </button>
          </div>
        </div>
      ...
      {showJoinModal && (
        <div className="modal-bg fi" onClick={e=>{if(e.target===e.currentTarget){setShowJoinModal(false);setJoinError("");}}}>
          <div className="modal">
            ...
            <input placeholder="Código del grupo" value={joinCode}
              onChange={e=>{setJoinCode(e.target.value);setJoinError("");}}
              onKeyDown={e=>e.key==="Enter"&&joinGroup()}
              autoFocus style={{ marginBottom: joinError?8:14, fontFamily:"monospace", letterSpacing:1 }} />
